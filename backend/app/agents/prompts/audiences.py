...
WRITING_STYLES = {
    ...
    "wechat_popular": {
        ...
        "emotion": "真实共情——可以代入读者视角，用自然口语拉近距离，允许适度幽默（如果它和主题沾边且帮助理解），但不刻意煽情，不堆砌网感套话",
        "ending": "实操建议清单或就医指引，可以有简短总结",
        "forbidden": "论文腔（'本文将探讨'）、[N]角标（公众号读者看不懂）、纯学术术语不解释、被动语态堆砌、名词化公文表达、没有人味的匀速行文、连续使用3个以上逻辑连接词（因此/然而/此外/同时）、反复使用同一种互动套话（如反复'敲黑板''有请主角'）",
        "typical": "'今天我们来聊聊' '大家应该都听说过' '简单理解就是'",
