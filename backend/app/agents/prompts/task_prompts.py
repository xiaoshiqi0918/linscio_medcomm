...
_SECTION_WORD_RATIOS: dict[str, dict[str, float]] = {
    "article": {"body": 0.48, "case": 0.17, "qa": 0.20, "summary": 0.12},
    "story": {
...
def _section_word_target(state: dict) -> str:
    ...
    cap = _SECTION_MAX_WC.get(fmt, {}).get(st)
    if cap and target > cap:
        target = cap

    lo = max(50, int(target * 0.8))
    hi = min(int(target * 1.2), cap) if cap else int(target * 1.2)
    return f"{lo}-{hi}字"
...
PLATFORM_HOOKS = {
    "wechat": "用一个具体的场景、数字或问题开头，避免套话",
    "xiaohongshu": "第一句给出最有用的具体信息，不要用'你知道吗'开头",
    "journal": "以一个有临床意义的现象或研究发现开头",
    "douyin": "第一句就是核心观点，不铺垫",
    "offline": "用一个具体的生活细节开头",
    "universal": "用一个让读者觉得'这跟我有关'的具体细节开头",
}
...
🚨🚨🚨 引言的定义——必须理解后再动笔 🚨🚨🚨
引言 ≠ 缩写版正文。引言是一扇门，让读者产生好奇走进来，仅此而已。
引言只做一件事：用 2-3 句话制造一个"认知缺口"——让读者意识到自己不知道某件事，想读下去。
引言绝不回答问题，不解释原因，不给结论，不展示证据。

【写法（三选一）】
A. 认知转折——先说习以为常的认知，一句话翻转
   例："拿到体检报告，看到某个指标飘红，大多数人的反应是紧张。但有时候，让你紧张的那个数字，可能根本就不是真的。"
B. 悬念切入——一个反常现象，点到为止
   例："一位各方面都很健康的女士，体检却查出甲状腺功能异常。复查正常了。再查又异常了。问题出在一个谁都没想到的地方。"
C. 直接提问——一个读者没想过但一听就在意的问题
   例："你有没有在体检后被告知某个指标异常、需要复查，但复查结果又完全正常？"

【关键原则】
- 只写 2-3 句话，50-100 字（绝不超过 100 字）
- 只写一段（ONE paragraph），一段写完引言就结束
- 留白比写满好——没说完的话就是正文存在的理由
- {platform_hooks}

语气：{audience['tone']}
句子：{audience['sentence']}

▌⛔ 禁止清单（违反任何一条 = 不合格，必须重写）
- ⛔ 禁止提及具体的物质名称、药物名称、疾病机制——这些是正文的工作
- ⛔ 禁止解释原因（如"这在医学上叫…"、"它主要分两类…"）
- ⛔ 禁止给出数据、百分比、剂量
- ⛔ 禁止引用文献 [N]
- ⛔ 禁止写超过一段——第二段直接删掉
- ⛔ 禁止"今天我们来聊聊""本文将介绍""接下来我们将探讨"
- ⛔ 禁止"守护健康""让我们一起"等宏大收尾
- ⛔ 禁止写超过 100 字
...
def get_article_body(state: dict) -> str:
    ...
    skip = set(state.get("skip_sections") or [])
    body_ratio = _SECTION_WORD_RATIOS.get("article", {}).get("body", 0.48)
    ...
    if body_budget <= 500:
        max_points = 2
        point_hint = "精选 2 个核心知识点，每个 100-130 字"
    elif body_budget <= 1500:
        max_points = 2
        point_hint = "精选 2 个核心知识点，每个 150-200 字，合计不超过 400 字"
    else:
        max_points = 3
        point_hint = "2-3 个核心知识点，每个 150-250 字"

    return f"""请为以下医学科普文章撰写正文。正文是文章的主体，包含开篇引入和核心知识讲解。

⚠️ 字数硬限制：本章节中文正文必须控制在 {wt} 之间。超出上限的内容会被系统截断。宁可少覆盖一个话题也不可超字。
...
【正文结构要求】
正文 = 引入段 + 知识讲解段落
1. 引入段（2-3句话）：用一个贴近读者生活的场景、疑问或反常识现象切入主题，制造好奇心，让读者想读下去
2. 知识讲解：紧接引入段，直接展开核心知识点的讲解，不再做任何二次引入

⚠️ 注意：正文不需要标题行。标题由系统自动生成，你直接输出正文内容即可。
...
类比：挑最难理解的 1 个概念用类比，其余直白讲清。{audience['analogy']}
禁止：{audience['forbidden']}

▌句式与段落
- 偶尔来一句特别短的打破节奏，禁止连续3句以上结构相同
- 段与段之间不要用连接词过渡（禁止"然而""因此""此外"等开头新段落）
- 全文保持同一个语域，不要交替切换口语和学术表达

▌内容
- {point_hint}——写完即止，不追加额外展开
- 每个要点一段，一段讲透一件事（机制/数据/误区三选一），不在一段内堆叠多个论据
- 有数据支撑时通俗表达（如"大约10个人中有1个"），无文献来源的数据不编造
- 首次出现的专业术语用通俗说法在前、术语在后

▌正文边界（硬性规则）
正文在最后一个要点讲完后立即结束。以下内容严禁出现在正文中：
- 行动建议 / 行动清单 → 留给小结
- 案例故事 → 留给案例章节
- 总结段 / 升华段 / "敲黑板"式总结 → 禁止
...
▌禁止
- 不给具体用药剂量、不使用绝对化表述
- ⛔ 不输出文章标题（标题由系统生成），第一行直接是引入段
- ⛔ 不在末尾附参考文献列表（系统自动处理），正文中只需 [1][2] 角标
- ⛔ 不在输出中标注字数统计（如"(84字)"），不输出任何元信息

请直接输出正文内容，第一行直接是正文段落。严格控制在 {wt} 以内。
"""
...
def get_article_case(state: dict) -> str:
    ...
    case_wt = _section_word_target(state) or '250-400字'
    return f"""请为以下医学科普文章撰写一个简短的典型案例。

⚠️ 字数硬限制：案例全文必须控制在 {case_wt} 之间。超出会被截断。
...
【案例写作规范】
案例必须虚构，用化名（如"王阿姨""小李"），年龄和职业符合目标读者。

案例只需讲清三件事：
① 这个人遇到了什么问题（2-3句）
② 问题怎么解决的（2-3句，医生/专家的核心解释通俗化）
③ 结果如何、读者能学到什么（1-2句，正向结尾）
...
▌禁止
- 不描述具体用药过程
- 不用"经过科学治疗，完全治愈"等过度乐观表述
- ⛔ 不在输出中标注字数统计，不输出任何元信息

请直接输出案例，严格控制在 {case_wt} 以内。
"""
...
def get_article_qa(state: dict) -> str:
    ...
    qa_ratio = _SECTION_WORD_RATIOS.get("article", {}).get("qa", 0.20)
    ...
    per_qa = min(100, max(50, qa_budget // 3))

    return f"""请为以下医学科普文章撰写常见问题解答（Q&A）部分。

⚠️ 字数硬限制：Q&A 全文必须控制在 {qa_wt} 之间。超出会被系统截断。
写 3 个 Q&A，每个问答（含问题和回答）控制在 {per_qa} 字以内。回答 3-5 句话即可，不要展开论述。
...
【核心原则】
- Q&A 补充正文未覆盖的读者疑惑，严禁重述正文已讲内容
- 不要引用或回顾前文内容，直接回答问题本身
- 用普通读者口吻提问（✅「能不能先自己控制饮食？」 ❌「是否可以通过饮食干预代替药物治疗？」）

【回答规范】
每题回答 3-5 句话，结构：① 结论（1句） ② 原因（1-2句） ③ 建议（1句）。写完立即停，不追加额外解释。
回答语气：{audience['tone']}
...
⚠️ 字数硬限制：小结全文必须控制在 {sum_wt} 之间。超出会被截断。
...
【小结 = 行动要点 + 就医提示，仅此而已】
- 开头不要回顾或概括前文，直接给出行动要点
- 不重复正文已讲的知识、类比、数据
- 不写升华段、感悟段、展望段

▌结构（写完就停）
① 行动要点（2-3 条，用"•"列表）
   - 每条一句话，动词开头，包含"做什么 + 怎么做"
   - ✅ 「化验前至少停服生物素 72 小时」
   - ❌ 「保持良好生活习惯」（空洞不可执行）
...
⛔ 就医提示写完后立即结束，不追加任何总结/感悟/升华内容。

风格：{audience['tone']}
禁止：套话开头（"总之""综上"）、空洞比喻、情感升华、"首先/其次/最后"并列
⛔ 不在输出中标注字数统计，不输出任何元信息

请直接输出小结内容，严格控制在 {sum_wt} 以内。
"""
