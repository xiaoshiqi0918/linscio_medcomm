...
_SECTION_WORD_RATIOS: dict[str, dict[str, float]] = {
    "article": {"body": 0.55, "case": 0.0, "qa": 0.22, "summary": 0.15},
    "story": {
...
字数限制：本章节中文正文必须控制在 {wt} 之间。宁可少覆盖一个话题也不可超字。
...
类比：挑最难理解的 1-2 个概念用类比，其余直白讲清。{audience['analogy']}
禁止：{audience['forbidden']}

▌句式节奏（极重要——直接决定是否被判为AI）
- 不追求均匀的长短句交替。连续几句差不多长是正常的，关键是偶尔来一句特别短的打破节奏
- 一句完整的话说完再打句号，不要把一句话拆成几个短句各打句号
- 句内需要解释时用冒号引出（如"原因很简单：……"），少用破折号
- 禁止连续3句以上结构相同（如连续三句都是"A可能由B导致"）

▌段落衔接（极重要——连接词密集是AI最大特征）
- 段与段之间不要用连接词过渡（禁止用"然而""因此""此外""与此同时""不仅如此"开头新段落）
- 段落衔接靠话题本身的逻辑关系：上一段说完A，下一段直接切入B，读者自己能接上
- 段内也少用逻辑连接词，靠信息本身的因果关系推进

▌风格一致性
- 全文保持同一个语域：要么口语化到底，要么书面到底
- 不要在同一段内交替切换口语和学术表达（如一句"卡了个BUG"紧接一句"竞争性地取代目标分子"）
- 比喻和解释要在同一个语域里

▌内容
- 围绕主题展开 {max_points} 个左右的要点，可以用小标题但不必每段都有
- 知识点之间的展开方式要有变化：有的先给数据再解释，有的先讲一个让读者意外的事实再说原因，有的直接列几个分类——不要每段都是"结论→解释→数据"的相同节奏
- 如果主题存在常见误区，在相关要点中自然纠正，不需要单独开"误区"章节
- 有争议的观点注明"目前主流观点认为"
- 有数据支撑时通俗表达（如"大约10个人中有1个"），无文献来源的数据不编造
- 首次出现的专业术语用通俗说法在前、术语在后的方式解释

▌正文边界
正文在最后一个要点讲完后自然结束。以下内容留给「小结」章节：
- 行动建议 / 行动清单
- 就医提示
- 情感收束 / 总结段 / 升华段
...
- 不使用「一定会」「绝对」等绝对化表述
- 不用「请记住」「总结一下」收尾
- ⛔ 不输出文章标题——标题由系统单独生成，正文第一行直接是引入段内容
- ⛔ 不在末尾附参考文献列表——参考文献由系统自动处理，正文中只需用 [1][2] 等角标

请直接输出正文内容。不要输出任何标题行（如"正文"或文章标题），不要附参考文献列表，第一行直接是正文段落。全文不超过 {wt}。
"""
...
🚨🚨🚨 最重要的规则——字数限制 🚨🚨🚨
小结全文必须控制在 {sum_wt} 之间。
行动要点 2-3 条（每条一句话）+ 就医提示 1-2 句，合计不超过 {sum_wt}。
⛔ 不写升华段、不写情感收束段、不写感悟段。行动要点+就医提示就是全部。
...
【前序衔接要求——小结是全文的总结，必须基于正文内容】
⚠️ 如果上方注入了「前序章节内容」，小结必须严格遵循：
1. 总结全文：行动要点必须从正文讲解的知识中提炼，不引入正文未提及的新知识
2. 回应主题：正文提出的核心问题，小结应给出简明的行动回应（而非重复解释）
3. 不重复正文：正文已详细展开的知识讲解、类比、数据，小结不得复述
4. 读者闭环：读者读完正文→小结后，应感到"我的问题被解答了，知道该怎么做了"
...
▌结构要求（按顺序，写完就停，总字数 {sum_wt}）

① 行动要点（2-3 条，占小结70%以上篇幅）
   - 每条一句话，动词开头
   - 每条必须是读者明天就能做的具体操作，包含：做什么 + 怎么做 + 关键数字（如果有）
   - ❌ 「保持良好生活习惯」「建立个人健康档案」「学会提问」（空洞不可执行）
   - ✅ 「化验前至少停服生物素补充剂 72 小时」「每天走路30分钟，分3次完成也可以」
   - 如果涉及临床操作的时间/剂量，必须有文献或指南依据
   - 注意：只涉及生活方式，不涉及用药
   - ⛔ 用"•"列表呈现，禁止用"首先/其次/最后"或"一是/二是/三是"并列
...
⛔⛔⛔ 就医提示写完后小结立即结束。
   不追加任何"总结段""感悟段""升华段""情感收束段""展望未来段"。
   就医提示本身就是自然的收尾。多写一句都是废话。⛔⛔⛔

▌语言要求
风格：{audience['tone']}
句长：{audience['sentence']}

▌禁止事项（⛔ 硬性红线，违反任何一条视为不合格）
- ⛔ 禁止"不是A而是B"句式（如"不是为了怀疑，而是为了更好地使用"）——直接说B
- ⛔ 禁止"搭档""密码""迷雾""真相""并肩""守护"等空洞比喻
- ⛔ 禁止情感升华和宏大叙事（如"让科学守护彼此""共同绘制健康蓝图""保持这份清醒的认知"）
- ⛔ 禁止"首先/其次/最后"并列——用"•"列表
- ⛔ 禁止"一方面/另一方面"对称结构
...
- 不展开解释正文已讲过的概念
- 健康建议不涉及具体药物
- 最后一句必须是具体的就医条件或操作建议，禁止以比喻/感悟/呼吁结尾

请直接输出小结内容，不要输出"小结"标题行，不要附参考文献列表。可以自行添加小标题。全文不超过 {sum_wt}。
"""
