    ...
    <el-form label-position="top" class="form">
      ...
      <el-form-item label="正向提示词">
        <el-input
          ...
          type="textarea"
          :rows="4"
          placeholder="留空则根据「场景」由系统自动构建英文提示词；填写则直接使用。"
        />
      </el-form-item>
      ...
      <el-form-item label="负向提示词">
        <el-input
          ...
          type="textarea"
          :rows="2"
          placeholder="留空：使用默认安全负向。填写：完全替换默认负向。"
        />
      </el-form-item>
      ...
      <el-collapse class="advanced-collapse">
        <el-collapse-item title="高级参数（Steps / CFG / 采样器）" name="adv">
          <div class="form-row-inline">
...
const aspectPresets = [
  ...
  { key: '3_4', label: '3:4 · 768×1024', w: 768, h: 1024 },
] as const
const aspectKey = ref<string>('1_1')
...
onMounted(async () => {
  try {
    ...
    if (list.length) {
      imageTypeOptions.value = list.map((t: any) => ({ id: t.id, name: t.name }))
      if (!list.some((t: any) => t.id === imageType.value)) {
...
async function handleAiPrompts() {
  ...
  try {
    const providerHint = activeProvider.value === 'midjourney' ? 'midjourney' : 'api'
